...
/*******************************************************************************
 INTRUSB:    Read USB interrupt register to know which interrupt has occurred.
 */
...
)
{
    USB->DMA_CHANNEL[dma_channel].CNTL |=
        ((ep_num << DMA_CNTL_REG_DMA_EP_NUM_SHIFT) & DMA_CNTL_REG_DMA_EP_NUM_MASK);
}
...
{
    USB->DMA_CHANNEL[dma_channel].CNTL |=
        ((burst_mode << DMA_CNTL_REG_DMA_BURST_MODE_SHIFT) & DMA_CNTL_REG_DMA_BURST_MODE_MASK);
}
...
{
    uint8_t mode;
    mode = (USB->DMA_CHANNEL[dma_channel].CNTL  & DMA_CNTL_REG_DMA_BURST_MODE_MASK);
...
/*lint -restore */
/*******************************************************************************
* ENDOF - Common function for DEVICE and HOST mode.
*/
